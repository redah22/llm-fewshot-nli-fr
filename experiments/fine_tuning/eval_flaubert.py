...
if mode == "eval_only":
    default_ckpt = f"models/flaubert_{dataset_name}"
    print(f"\nChemin du checkpoint (défaut: {default_ckpt})")
    print("Exemple: models/flaubert_gqnli_fr")
    print("Ou tapez 'base' pour tester FlauBERT sans fine-tuning.")
    ckpt_path = input("Chemin: ").strip()

    if not ckpt_path:
        ckpt_path = default_ckpt

    if ckpt_path.lower() in ["base", "flaubert-base", "flaubert/flaubert_base_cased"]:
    ...
    print("\n" + "="*40)
    print("MODÈLE DE BASE (Transfer Learning)")
    print("="*40)
    print("Quel modèle de base voulez-vous fine-tuner ?")
    print("1. flaubert/flaubert_base_cased (Par défaut)")
    print("2. Un modèle pré-entraîné localement (ex: models/flaubert_xnli_fr)")
    mod_choice = input("\nVotre choix (1 ou 2) [défaut=1]: ").strip()
    
    model_name = 'flaubert/flaubert_base_cased'
    if mod_choice == "2":
        custom_path = input("Entrez le chemin vers le modèle (ex: models/flaubert_xnli_fr) : ").strip()
        if os.path.exists(custom_path):
            model_name = custom_path
            print(f"✅ Modèle personnalisé sélectionné: {model_name}")
        else:
            print(f"⚠️ Chemin introuvable: {custom_path}. Retour au modèle par défaut.")
    else:
        print(f"✅ Modèle par défaut sélectionné: {model_name}")
...
# Set format — FlauBERT a besoin de token_type_ids
...
# En mode same_dataset, évaluer sur le test hold-out
...
if final_eval_dataset:
    ...
    is_baseline_run = (mode == "eval_only" and model_name == "flaubert/flaubert_base_cased")
    ...
    if custom_result_name:
        ...
        result_filename = f'results/flaubert_{dataset_name}_{mode}{baseline_suffix}.json'
...
# (Note: we should just tell the user how to run it in documentation, changing eval_flaubert.py to ask for the base model path is a good improvement)
